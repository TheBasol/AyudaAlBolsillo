...
export async function GET(request: Request) {
    ...
    await prisma.session.create({
      data: {
        userId: adminUser.id,
        token: "admin-token-example-1234567890",
    ...
    await prisma.session.create({
      data: {
        ...
        token: "user-token-example-0987654321",
        expiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // 7 días
      }
    });
    ...
    await prisma.subscription.create({
      data: {
        ...
        endDate: new Date(Date.now() + 335 * 24 * 60 * 60 * 1000), // 335 días adelante
        isActive: true,
        paymentMethod: "CREDIT_CARD",
        lastPaymentDate: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
        nextPaymentDate: new Date(Date.now() + 335 * 24 * 60 * 60 * 1000),
      }
    });
    ...
    await prisma.subscription.create({
      data: {
        ...
        planType: "BASIC",
        startDate: new Date(Date.now() - 15 * 24 * 60 * 60 * 1000), // 15 días atrás
        endDate: new Date(Date.now() + 15 * 24 * 60 * 60 * 1000), // 15 días adelante
        isActive: true,
